...
local _RCFC_meta = {
    ...
    -- @ADD by cream
    MergeFrom = function(self, fields)
        -- repeated 从 1 开始, 只MergeFrom repeated 数据, 因此使用 ipairs
        for field, value in ipairs(fields) do
            local new_val = self:add()
            new_val:MergeFrom(value)
        end
    end,
    CopyFrom = function(self, fields)
    -- repeated 从 1 开始, 只CopyFrom repeated 数据, 因此使用 ipairs
        for field, value in ipairs(fields) do
            local new_val = self:add()
